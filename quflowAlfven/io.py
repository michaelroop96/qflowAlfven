...
class QuData(object):
    ...
    def __call__(self, W,theta,chi, qtime=0.0, **kwargs):
        ...
        if self.cache_size == 1:
            ...
            if self.W_cache is None:
                ...
                self.Chi_cache = np.zeros((self.cache_size,) + chi.shape, dtype=chi.dtype)
            ...
            self.Chi_cache[self.cache_steps, :, :] = chi
    ...
    def flush(self):
        ...
        if self.cache_steps != 0:
            save(self.filename, self.W_cache[:self.cache_steps], self.Theta_cache[:self.cache_steps],self.Chi_cache[:self.cache_steps],qtime=self.qtime_cache[:self.cache_steps],
    ...
    def _save_attrs(self, save_cache=False):
        with h5py.File(self.filename, "a") as f:
            ...
            if save_cache:
                ...
                if self.datapath+'Chi_cache' not in f:
                    f.create_dataset(self.datapath+'Chi_cache', self.Chi_cache.shape, dtype=self.Chi_cache.dtype)
                ...
                f[self.datapath+'Chi_cache'][...] = self.Chi_cache

            f[self.datapath].attrs.update(attrs)


def save(filename, data, data1,data2, qtime=None, qstepsize=None, N=None, qtype="shr", datapath="/", attrs=None):
    """
        ...
        if qtype == "shr" or qtype == "shc":
            ...
            if data_qtype == "shr" or data_qtype == "shc":
                ...
                for d in data2:
                    if qtype == "shc":
                        if d.ndim == 2:
                            omega2.append(mat2shc(d))
                        else:
                            raise ValueError("Something wrong with the input data.")
                    else:
                        omega2.append(as_shr(d))
            ...
            if statepath1 not in f:
                ...
                f[datapath].attrs["created"] = datetime.datetime.now().isoformat()
                stateset.attrs["qtype"] = qtype
            elif f[statepath1].shape[-1] != omega1.shape[-1] or f[statepath1].ndim != 2:
                raise ValueError("The file qtype does not seem to be correct.")
            if statepath2 not in f:
                stateset2 = f.create_dataset(statepath2, (0, omega2.shape[-1]),
                                            dtype=omega2.dtype,
                                            maxshape=(None, omega2.shape[-1]),
                                            chunks=(1, omega2.shape[-1])
                                            )
                if N is not None:
                    assert N == round(np.sqrt(omega2.shape[-1]))
                else:
                    N = round(np.sqrt(omega2.shape[-1]))
                f[datapath].attrs["N"] = N
                f[datapath].attrs["version"] = __version__
                f[datapath].attrs["created"] = datetime.datetime.now().isoformat()
                stateset.attrs["qtype"] = qtype
            elif f[statepath2].shape[-1] != omega2.shape[-1] or f[statepath2].ndim != 2:
                raise ValueError("The file qtype does not seem to be correct.")
            ...
            f[statepath2].resize(f[statepath2].shape[0]+omega2.shape[0], axis=0)
            ...
            f[statepath2][-omega2.shape[0]:, :] = omega2
            ...
            f[statepath2].attrs["modified"] = modtime.isoformat()
