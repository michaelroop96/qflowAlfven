...
def isomp_fixedpoint(W,theta,chi,alpha, stepsize=0.1, steps=100, hamiltonian=solve_poisson, forcing=None,
                      tol=1e-8, maxit=5, verbatim=True):
    """
    Time-stepping by isospectral midpoint second order method for skew-Hermitian W
    using fixed-point iterations.
    ...
    chi0 = np.zeros((N,N),dtype=np.complex128)
    Psi = W-chi
    for k in range(steps):
        ...
        for i in range(15):
            ...
            M1 = solve_poisson(psi0+chi0)
            M2 = laplace(theta0)
            M3 = M1-alpha*chi0
            
            rhsPsi = (np.eye(N)+h/2*M1)@psi0@(np.eye(N)-h/2*M1)
            psi1 = psi0-(-Psi+rhsPsi)
            
            rhsTheta = (np.eye(N)+h/2*M3)@theta0@(np.eye(N)-h/2*M3)
            theta1 = theta0-(-theta+rhsTheta)
            
            rhsChi = (np.eye(N)+h/2*M3)@chi0@(np.eye(N)-h/2*M3)-h/2*LieBracket(theta0,M2)-h**2/4*(M2@theta0@M3+M3@theta0@M2)
            chi1 = chi0-(-chi+rhsChi)
        ...
        tildeTheta = theta1
        tildeChi = chi1
        tildePsi = (tildePsi-tildePsi.conj().T)/2
        tildeTheta = (tildeTheta-tildeTheta.conj().T)/2
        tildeChi = (tildeChi-tildeChi.conj().T)/2
        M1 = solve_poisson(tildePsi+tildeChi)
        M2 = laplace(tildeTheta)
        M3 = M1-alpha*tildeChi
        
        Psi = Psi+h*LieBracket(tildePsi, M1)
        theta = theta+h*LieBracket(tildeTheta,M3)
        chi = chi+h*LieBracket(tildeChi, M3)+h*LieBracket(tildeTheta,M2)
        W = Psi+chi
